...
(function () {
    ...
    window.Framework7 = function (params) {
        ...
        ************   Week Calendar   ************
        ======================================================*/
        var weekCalendar = function (params) {
            var p = this;
            var defaults = {
                weekLayout: true,
            ...
            // Calendar Methods
            p.weekLayoutToggle = function(){
                p.params.weekLayout = !p.params.weekLayout;
                console.log(p.params.weekLayout);
                return p.params.weekLayout;
            };
            ...
            p.monthHTML = function (date, offset) {
                ...
                var year = date.getFullYear(),
                    ...
                    day = date.getDate();
                if (offset === 'next') {
                    //Lägg till felhantering på slutet av månader och år
                ...
                var daysInPrevMonth = p.daysInMonth(new Date(date.getFullYear(), date.getMonth()).getTime() - 10 * 24 * 60 * 60 * 1000), //hur många dagar det var i den föregående månaden
                    daysInMonth = p.daysInMonth(date), //Hur många dagar det är i nuvarande månaden
                    firstDayOfMonthIndex = new Date(date.getFullYear(), date.getMonth()).getDay(); //index på första veckodagen i månaden, ex börjar det på en onsdag blir det 3
                //if (firstDayOfMonthIndex === 0) firstDayOfMonthIndex = 7;

                
                var dayDate, currentValues = [], i, j, k,
                    cols = 7, rows = p.params.weekLayout ? 1 : 6,
                    monthHTML = '',
                    dayIndex = 0 + (p.params.firstDay - 1), // = 1 ty firstDay = monday dvs 1
                    today = new Date().setHours(0,0,0,0),
                    minDate = p.params.minDate ? new Date(p.params.minDate).getTime() : null, //om det finns en min-datum blir minDate det annars null
                ...
                for (i = 1; i <= rows; i++) {
                    ...
                    for (j = 1; j <= cols; j++) {
                        var col = j;
                        dayIndex ++; //Börjar på 1 och går upp till cols dvs 7

                        var dayNumber = dayIndex - firstDayOfMonthIndex; //dagen i veckan förhållande till startdatumet på månaden
                        //Om vi är på sista dagen i veckan börjar vi om med weekDayIndex = 0; Annars är weekDayIndex = col;
                        var weekDayIndex = (col - 1 + p.params.firstDay > 6) ? (col - 1 - 7 + p.params.firstDay) : (col - 1 + p.params.firstDay);
                        var addClass = ''; //extra class
                        if (dayNumber < 0) {
                        //Om vi är innan startdatumet i månaden i veckan blir dayNumber datumet i förra månaden.
                        //Tex börjar en månad på en tisdag kommer måndag ha dayNumber = sista datumet i föregående månad ty dayNumber = -1 tidigare
                            dayNumber = daysInPrevMonth + dayNumber + 1;
                            addClass += ' picker-calendar-day-prev';
                            //Sätter dayDate till datumet på veckodagen (om vi är i första veckan i januari kommer year-- samt month = december dvs 11)
                    ...
                    monthHTML += '<div class="picker-calendar-row calendar-row">' + rowHTML + '</div>';
                }
        ...
        app.weekCalendar = function (params) {
            return new weekCalendar(params);
        };
